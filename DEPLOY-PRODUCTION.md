...
в том числе правки, которые владелец витрины позже внёс через обычный UI. Никаких
других аккаунтов, бронирований, отзывов и чатов он не создаёт.

> ⚠️ **Но это НЕ «однажды отработал и навсегда no-op».** Каталог витрины не
> захардкожен в самом раннере: `DemoContentBootstrapRunner` берёт данные из
> `DevelopmentSeedData.Listings`, фильтруя их по `Status == Approved`
> (см. строку `var approvedSeeds = DevelopmentSeedData.Listings...`). No-op он
> ровно до тех пор, пока этот **dev-сид не пополнили**. Как только в
> `DevelopmentSeedData` добавляют новые Approved-объявления, ближайший деплой
> с `BOOTSTRAP_DEMO_CONTENT_ENABLED=true` **автоматически опубликует их на
> проде** — от имени владельца витрины, без отдельного решения и без миграции.
>
> Так и произошло 2026-08-14: коммит `e712cdb` (50-listing toy-catalogue
> expansion) расширил dev-сид, и деплой добавил на прод **44 новых объявления**
> (`Demo content bootstrap completed. Owner created: False, listings: 44,
> images: 0`). Пять аккаунтов `@toyrent.am` при этом на прод НЕ попали —
> раннер вешает все объявления на единственного владельца витрины, а
> пользователей из dev-сида не создаёт вовсе.
>
> Практический вывод для деплоя: «сид — только для Development» верно для
> `SeedDevelopmentDataAsync()` (он под `IsDevelopment()`), но **не** для
> контента объявлений. Перед каждым деплоем проверять diff
> `DevelopmentSeedData.cs`, а не только файлы с `DemoContent`/`Bootstrap` в
> имени — иначе изменение каталога прода останется незамеченным:
>
> ```bash
> git diff --stat <deployed>..origin/main -- '*DevelopmentSeedData*'
> ```
>
> Если пополнять каталог прода в этот деплой не планируется — выставить
> `BOOTSTRAP_DEMO_CONTENT_ENABLED=false` в `/opt/dorent/rental-api/.env` до
> `up -d`. Уже созданные объявления от этого не исчезнут (раннер только
> добавляет).
