...
(Ubuntu, IP `91.98.170.128`) через Cloudflare Tunnel. Сервер — 2 ГБ RAM
...
```bash
rsync -avz --exclude .git rental-api/  dorent@91.98.170.128:/opt/dorent/rental-api/
rsync -avz --exclude .git Rental-Ui/   dorent@91.98.170.128:/opt/dorent/Rental-Ui/
```
