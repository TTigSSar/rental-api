> # ⛔ НЕ РАЗВЁРНУТО — ЭТО ПЛАН, А НЕ ОПИСАНИЕ РЕАЛЬНОСТИ
>
> **Staging-стенда не существует. Он никогда не запускался.**
> `test.dorent.am` **не резолвится** (NXDOMAIN): в Cloudflare Tunnel опубликованы
> только `dorent.am` и `www.dorent.am` → `http://ui:80`. Маршрута на staging нет.
> `docker-compose.staging.yml` не запускался ни разу — на VPS работает **только
> production** (`docker-compose.production.yml`, `ASPNETCORE_ENVIRONMENT=Production`).
>
> **Staging не помещается на текущий VPS.** 2 ГБ RAM (Hetzner CPX12) уже заняты
> production-стеком: SQL Server ограничен 1 ГБ, для сборки образов нужен swap.
> Второй стенд — это второй полный стек, включая второй SQL Server (≈1 ГБ);
> OOM killer с высокой вероятностью снесёт **production-базу**. Плюс конфликт
> портов: `127.0.0.1:8080` и `127.0.0.1:4200` уже заняты production.
>
> **Тестовая среда с сидом — это локальный `docker-compose.yml`** (в корне
> `rental-api`), а не удалённый стенд. Ходить некуда: смотреть нечего.
>
> **Чтобы оживить этот документ**, нужен второй VPS или рескейл до 4 ГБ RAM.
> Решение и отвергнутые альтернативы — **ADR-004** в `knowledge/decisions.md`.
> Файл не удалён намеренно: он остаётся рабочей отправной точкой, если стенд
> когда-нибудь профинансируют. Всё, что ниже, — **инструкция на будущее**.
